...
import {Vector3, Quaternion, PerspectiveCamera, Math as math, EventDispatcher} from 'three'
...
function StereoEffect (renderer) {
	...
	Object.defineProperties( this, {
		separation: {
			...
			set: function ( value ) {
				console.warn( 'StereoEffect: .separation is now .eyeSeparation.' );
		...
		targetDistance: {
			...
			set: function ( value ) {
				console.warn( 'StereoEffect: .targetDistance is now .focalLength.' );
	...
	this.render = function ( scene, camera ) {
		...
		_fov = math.radToDeg( 2 * Math.atan( Math.tan( math.degToRad( camera.fov ) * 0.5 ) / camera.zoom ) );
		_ndfl = camera.near / this.focalLength;
		_halfFocalHeight = Math.tan( math.degToRad( _fov ) * 0.5 ) * this.focalLength;
...
StereoEffect.prototype = Object.create(EventDispatcher.prototype );
